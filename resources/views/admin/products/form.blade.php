...
{{-- Slug sản phẩm --}}
<x-admin.input name="slug" label="Slug" :value="$product->slug ?? ''" />
...
<x-admin.select name="user_id" label="User" :options="$users" :selected="$product->user_id ?? null" />

{{-- Danh mục sản phẩm (Select Component) --}}
<x-admin.select name="category_id" label="Category" :options="$categories->pluck('name', 'id')" :selected="$product->category_id ?? null" />
